...
and checkExp  (ftab : FunTable)
    ...
    *)
    ...
    | Not (e, pos) ->
        ...
        if e_type <> Bool then reportTypeWrong "Operand of not" Bool e_type pos
        (Bool, Not (e_dec, pos))

    | Negate (e, pos) ->
        let (e_type, e_dec) = checkExp ftab vtab e
        if e_type <> Int then reportTypeWrong "Operand of negate"  Int e_type pos
    ...
    *)
    ...
    (Array elem_type, Scan (f', ne_dec, arr_dec, elem_type, pos))
